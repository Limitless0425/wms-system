<template>
  <div class="mobile-page">
    <div class="topbar">
      ...
      <strong>扫码入库</strong>
      <el-button link type="primary" @click="$router.push('/mobile/inbound-orders')">入库单</el-button>
    </div>

    <el-card>
      <el-input v-model="kanbanNo" size="large" placeholder="输入或扫描入库看板号" @keyup.enter="query" />
      <div class="btn-row">
    ...
    <el-card v-if="kanban.id" class="info-card">
      <template #header>
        <div class="card-header">
          ...
          <el-tag :type="kanban.sealed ? 'danger' : kanban.status === 'PRINTED' ? 'warning' : 'success'">
            {{ kanban.sealed ? '已封存' : kanban.status === 'PRINTED' ? '待入库' : '已入库' }}
          </el-tag>
        </div>
      </template>
      ...
      <el-input-number v-model="scanQty" :min="1" :max="kanban.qty" class="qty" />
      <el-button class="full-btn" type="success" size="large" :disabled="kanban.status !== 'PRINTED' || kanban.sealed" @click="confirmScan">确认入库</el-button>
    </el-card>

    <el-dialog v-model="cameraVisible" title="摄像头扫码" width="92%" destroy-on-close @closed="stopCamera">
      <div id="mobile-inbound-reader" class="camera-reader" />
...
import { getKanbanByNo, scanInbound } from '@/api/kanban'
...
async function query() {
  ...
  try {
    ...
    scanQty.value = kanban.value.qty
  } catch {
    kanban.value = {}
  }
}
async function confirmScan() {
  await scanInbound(kanban.value.kanbanNo, { qty: String(scanQty.value), scanner: 'mobile' })
...
async function openCamera() {
  ...
  await nextTick()
  try {
    scanner = new Html5Qrcode('mobile-inbound-reader')
