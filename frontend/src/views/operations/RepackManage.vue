<template>
  <div class="page-container">
      ...
      <el-form :inline="true">
        <el-form-item label="状态">
          <el-select v-model="searchForm.status" clearable placeholder="选择" style="width:130px" @change="loadOrders">
            <el-option label="待转包" value="PENDING" /><el-option label="转包中" value="PROCESSING" /><el-option label="已完成" value="COMPLETED" />
            <el-option label="已作废" value="VOIDED" />
          </el-select>
        </el-form-item>
        <el-form-item label="转包单号"><el-input v-model="searchForm.orderNo" placeholder="请输入转包单号" clearable style="width:170px" @keyup.enter="loadOrders" /></el-form-item>
        <el-form-item label="供应商">
          <el-select v-model="searchForm.supplier" placeholder="请选择供应商" clearable style="width:180px" @change="loadOrders">
        ...
        <el-form-item><el-button type="success" @click="openCreate">新建转包单</el-button></el-form-item>
      </el-form>
    </el-card>
      ...
      <el-table :data="pagedOrders" border stripe @selection-change="selectedOrders = $event">
        ...
        <el-table-column label="余量" width="90">
          <template #default="{ row }">
            <el-tag :type="row.allowBalance === false ? 'info' : 'success'">{{ row.allowBalance === false ? '不允许' : '允许' }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="360" fixed="right">
          <template #default="{ row:r }">
            <el-button size="small" type="success" @click="printOrder(r)">打印转包单</el-button>
            <el-button size="small" type="primary" @click="viewKanbans(r)">打印看板</el-button>
    ...
    <el-dialog title="关联转包看板" v-model="kanbanVisible" width="1200px">
    ...
    <el-dialog title="新建转包单" v-model="dialogVisible" width="1300px" @close="onDialogClose">
      <el-form :model="form" label-width="70px" :inline="true">
        ...
        <el-form-item label="带余量">
          <el-switch v-model="form.allowBalance" active-text="允许" inactive-text="不允许" />
        </el-form-item>
      ...
      <el-collapse v-if="false" style="margin-bottom:10px">
        <el-collapse-item title="批量导入物料（粘贴文本）">
          <el-input v-model="batchText" type="textarea" :rows="4" placeholder="一行一种零件，格式：零件编码,数量,仓库编码,器具编码,库位编码" />
      ...
      <el-table v-if="false" :data="partList" border stripe max-height="400" @selection-change="onSelectionChange" ref="partTableRef">
        ...
        <el-table-column label="数量"><template #default="{ row }"><el-input-number v-model="row.planQty" :min="1" size="small" style="width:100px" /></template></el-table-column>
        <el-table-column label="器具" width="160"><template #default="{ row }"><el-select v-model="row.containerCode" placeholder="选择器具" size="small" style="width:140px"><el-option v-for="c in containers" :key="c.id" :label="c.code+' '+c.name" :value="c.code" /></el-select></template></el-table-column>
...
import { getRepackOrders, createRepackOrder, deleteRepackOrder, executeRepackOrder, getRepackOrderKanbans } from '@/api/operations'
import { getSuppliers, getParts, getWarehouses, getLocations as fetchLocations, getContainers } from '@/api/baseInfo'
...
const searchForm = reactive({ status: '', orderNo: '', supplier: '' })
const suppliers = ref([])
...
const form = reactive({ supplierId: null, repackDirection: '向下转包', allowBalance: true, remark: '', items: [] })
const partList = ref([])
...
const draftKey = 'repack-order-item-draft'
...
onMounted(async () => {
  const [supplierRes, warehouseRes, locationRes, containerRes] = await Promise.all([
    getSuppliers(), getWarehouses(), fetchLocations(), getContainers()
  ])
...
async function loadOrders() {
  const res = await getRepackOrders(searchForm)
...
function openCreate() {
  ...
  form.repackDirection = '向下转包'
  form.allowBalance = true
  form.remark = ''
  form.items = []
  partList.value = []
  selectedParts.value = []
...
function openBatchPicker() {
  ...
  router.push({
    ...
    query: { type: 'repack', supplierId: form.supplierId, draftKey, returnName: 'RepackManage', qtyLabel: '转包数量' }
  })
}
...
async function restoreDraftFromRoute() {
  ...
  dialogVisible.value = true
  router.replace({ name: 'RepackManage' })
}

function onWhChange(row) { row.locationId = null }
...
function parseBatch() {
  ...
  for (const line of lines) {
    ...
    const containerCode = parts[3] || ''
    const locCode = parts[4] || ''
...
async function handleSave() {
  ...
  const items = partList.value
    ...
    .map(p => {
      ...
      return {
        ...
        targetContainerType: p.targetContainerType,
        originalPackageQty: p.originalPackageQty,
        targetPackageQty: p.targetPackageQty,
        warehouseId: p.warehouseId, warehouseName: wh ? wh.name : '',
        locationId: p.locationId, locationName: l ? l.code : ''
      }
    })
  if (!items.length) { ElMessage.warning('请勾选至少一种零件'); return }
  const insufficient = partList.value.find(item => Number(item.availableQty || 0) <= 0 || Number(item.planQty || 0) > Number(item.availableQty || 0))
  if (insufficient) {
    ElMessage.warning(`零件 ${insufficient.partCode || insufficient.code} 可转包库存不足，当前可用 ${insufficient.availableQty || 0}，计划转包 ${insufficient.planQty || 0}`)
    return
  }
  const splitWarnings = items.map(containerSplitText).filter(Boolean)
  if (splitWarnings.length) {
    await ElMessageBox.confirm(
      `以下转包数量超过单个器具容量，系统会按容量自动拆成多张转包看板：\n${splitWarnings.join('\n')}`,
      '器具容量提醒',
      { type: 'warning' }
    )
  }
  form.items = items
  try {
    await createRepackOrder(form)
    ElMessage.success('转包单创建成功，已自动生成转包看板')
    dialogVisible.value = false
    await loadOrders()
  } catch (e) { ElMessage.error(e.response?.data?.message || '操作失败') }
}

function containerSplitText(item) {
  const container = containers.value.find(c => c.code === item.containerCode)
  const capacity = Number(item.targetPackageQty || container?.capacity || 0)
  const qty = Number(item.planQty || 0)
  if (!capacity || qty <= capacity) return ''
  const count = Math.ceil(qty / capacity)
  const remainder = qty % capacity
  const fillText = remainder === 0 ? '全部装满' : `最后一个未装满（${remainder}/${capacity}）`
  return `${item.partCode || item.partName}：数量 ${qty}，器具容量 ${capacity}，需要 ${count} 个，${fillText}`
}
...
async function handleDelete(r) {
  await ElMessageBox.confirm('确定删除? 相关看板将一并删除', '提示', { type: 'warning' })
  await deleteRepackOrder(r.id)
  ElMessage.success('已删除')
  await loadOrders()
}
async function handleBatchDelete() {
  if (!selectedOrders.value.length) return
  await ElMessageBox.confirm(`确定批量删除 ${selectedOrders.value.length} 条转包单吗？`, '批量删除', { type: 'warning' })
  for (const row of selectedOrders.value) await deleteRepackOrder(row.id)
...
function printOrder(r) {
  viewKanbans(r)
  setTimeout(() => {
    if (currentKanbans.value.length) kanbanTableRef.value?.toggleAllSelection()
  }, 500)
}

async function viewKanbans(r) {
  const data = (await getRepackOrderKanbans(r.id)).data
...
function batchPrintKanbans() {
  ...
  const cards = selectedKanbans.value.map(k => '<section class="kanban-card"><h2>转包看板</h2><img src="' + k.qrDataUrl + '" alt="' + escapeHtml(k.kanbanNo) + '"><p class="kanban-no">' + escapeHtml(k.kanbanNo) + '</p><table><tr><td>零件</td><td>' + escapeHtml(k.partCode) + ' ' + escapeHtml(k.partName) + '</td></tr><tr><td>数量</td><td>' + escapeHtml(k.qty) + ' ' + escapeHtml(k.unit||'') + '</td></tr><tr><td>目标器具</td><td>' + escapeHtml(k.targetContainerName||'-') + '</td></tr><tr><td>仓库</td><td>' + escapeHtml(k.warehouseName||'-') + '</td></tr><tr><td>库位</td><td>' + escapeHtml(k.locationName||'-') + '</td></tr><tr><td>转包单</td><td>' + escapeHtml(k.orderNo||'-') + '</td></tr></table></section>').join('')
  const w = window.open('', '_blank')
  if (!w) { ElMessage.warning('浏览器阻止了打印窗口'); return }
  w.document.write('<!doctype html><html><head><title>批量打印转包看板</title><style>body{margin:0;font-family:SimSun,serif}.kanban-card{box-sizing:border-box;width:100%;padding:24px;text-align:center;page-break-after:always}.kanban-card:last-child{page-break-after:auto}.kanban-card img{width:200px;height:200px}.kanban-no{font-size:18px;font-weight:bold}table{width:100%;border-collapse:collapse}td{border:1px solid #333;padding:8px;text-align:left}td:first-child{width:25%;text-align:center}</style></head><body>' + cards + '</body></html>')
  w.document.close(); w.onload = () => { w.focus(); w.print() }
}

function sl(v) { return { PENDING: '待转包', PROCESSING: '转包中', PARTIAL: '转包中', COMPLETED: '已完成', VOIDED: '已作废' }[v] || v }
function st(v) { return { PENDING: 'warning', PROCESSING: 'primary', PARTIAL: 'primary', COMPLETED: 'success', VOIDED: 'danger' }[v] || 'info' }
</script>
